...
class BasePlugin {
    ...
    public function addScript(string $path, array $dependencies = array(), bool $inFooter = \false, bool $version = \false) {
    ...
    public function addRestRoute(string $method, string $route, string $endpoint, callable $callback) {
        add_action('rest_api_init', function () use(&$method, &$callback, $route, &$endpoint){
                ...
                function () use($HTMLFieldNames, $HTMLSubmitButtonName, $HTMLUiPath) {
                    ...
                    if (isset($_POST[$HTMLSubmitButtonName])) {
                        foreach($HTMLFieldNames as $fieldName) {
                            if(isset($_POST[$fieldName])) { $updateFields[] = sanitize_text_field($_POST[$fieldName]);
                            }
                        }
